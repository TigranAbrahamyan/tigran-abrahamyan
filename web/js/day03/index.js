...
//  ...
//  ]
const employeeBonus = (data) => data
  .filter((n) => Number(n.hr.salary.replace(/\D/g, '')) <= 100000)
  .map((n) => ({ ...n, hr: { ...n.hr, bonus: `$${Number(n.hr.salary.replace(/\D/g, '')) * 20 / 100}` } }));

console.log(employeeBonus(parsedData));
...
//       ...
//    }
const numberEmployeesByCity = (data) => data.reduce((acc, n) => ((acc[n.contact[0]] = acc[n.contact[0]] ? acc[n.contact[0]] + 1 : 1), acc), {});
console.log(numberEmployeesByCity(parsedData));
...
//   c) Հետո պատահական վերցնել 1 աշխատակից և ջնջել զանգվածից: ( Math.random() )
//   d) Տպել նոր զանգված բոլոր աշխատակիցների անուններով։
//
//   output
//   BOSS: Սպասեք մտածեմ
//   Մեր բոլոր աշխատակիցները։ Tiger Nixon, Vivian Harrell, Sonya Frost
//   Ներող ապեր քեզ դուրս ենք հանում: Vivian Harrell
//   Նոր աշխատակիցների ցուցակ։ Tiger Nixon, Sonya Frost
const removeEmployee = (data) => {
  let employeesData = data;
  console.log('BOSS: Սպասեք մտածեմ');

  setTimeout(() => {
    const employeeNames = employeesData.map((n) => n.name);
    const randomEmployee = employeeNames[Math.floor(Math.random() * employeeNames.length)];

    console.log('Մեր բոլոր աշխատակիցները։', employeeNames.join(', '));
    console.log('Ներող ապեր քեզ դուրս ենք հանում:', randomEmployee);

    employeesData = employeesData.filter((n) => n.name !== randomEmployee);
    const newEmployeeNames = employeesData.map((n) => n.name);

    console.log('Նոր աշխատակիցների ցուցակ։', newEmployeeNames.join(', '));
  }, 1000);
};

removeEmployee(parsedData);
